...
if (rootPackageJson.scripts) {
    ...
    lifecycleScripts.forEach((script) => {
        if (rootPackageJson.scripts[script]) {
            console.log(
                `- Root package has ${script} script: ${rootPackageJson.scripts[script]}`
            );
        }
    });
}
...
console.log(
    "1. Consider using npm cache with GitHub Actions for faster installations"
);
console.log(
    '2. Ensure NODE_OPTIONS="--max-old-space-size=4096" is set for large projects'
);
console.log(
    "3. Check npm version compatibility (current version:",
...
console.log(
    "6. Use direct directory changes instead of workspace flags if issues persist"
);
