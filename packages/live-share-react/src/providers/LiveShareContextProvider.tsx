...
> = (props) => {
  ...
  const clientRef = React.useRef(new LiveShareClient(props.host, props.clientOptions));
  ...
  const joinContainer = React.useCallback(
    async (
      ...
      const results: ILiveShareContainerResults = await clientRef.current.joinContainer(
