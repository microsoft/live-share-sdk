...
import { LiveEvent, LivePresence, LivePresenceUser, PresenceState, UserMeetingRole } from "@microsoft/live-share";
 ...
 */
export const usePresence = (acceptPlaybackChangesFrom: UserMeetingRole[], presence?: LivePresence, context?: app.Context) => {
  ...
  const localUserIsEligiblePresenter = useMemo(() => {
    ...
    return (
      localUserRoles.filter((role) => acceptPlaybackChangesFrom.includes(role))
        .length > 0
    );
  ...
  useEffect(() => {
    ...
    if (presence && !presence.isInitialized && context) {
      ...
      presence.on("presenceChanged", (userPresence: LivePresenceUser<IUserData>, local) => {
        ...
        const userArray = presence.toArray() as LivePresenceUser<IUserData>[];
        setUsers(userArray);
      });
