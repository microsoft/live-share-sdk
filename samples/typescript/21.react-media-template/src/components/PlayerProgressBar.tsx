...
const PlayerProgressBar: FC<{
...
}> = ({ currentTime, duration, isPlaybackDisabled, onSeek }) => {
    ...
    const onMouseMove: ReactEventHandler<HTMLElement> = (
        e: SyntheticEvent<HTMLElement, MouseEvent>
    ) => {
        if (dimension) {
            const xPosition = Math.min(
                Math.max(e.nativeEvent.clientX, dimension.left),
                dimension.right - 4
            );
            ...
            toolTipPositioningRef.current?.setTarget({
                getBoundingClientRect: getRect(
                    xPosition,
                    dimension.top - scrollOffSet
                ),
            });
        }
    };

    const onTouchMove: ReactEventHandler<HTMLElement> = (
        e: SyntheticEvent<HTMLElement, TouchEvent>
    ) => {
        if (dimension) {
            const xPosition = Math.min(
                Math.max(e.nativeEvent.touches[0].clientX, dimension.left),
                dimension.right - 4
            );
            const distanceFromOrigin = xPosition - dimension.left;
            const mousePos = (distanceFromOrigin / dimension.width) * 100;
        ...
        <div className={flexItemStyles.noShrink} ref={resizeRef}>
            <div className={styles.pageEl}>
                <Tooltip
                    ...
                    positioning={{
                        positioningRef:
                            toolTipPositioningRef as MutableRefObject<PositioningImperativeRef>,
                    }}
